...
def divide_three(first, second, third):
...
def get_num(string):
    try:
        ...
        print("I can't parse your input. Please enter a number next time")
        sys.exit()
...
def main():
    ...
    num_operands = get_num(input('2 or 3 operands: '))

    # get the first and second operands
    operand_1 = get_num(input('Please enter the first operand: '))
    operand_2 = get_num(input('Please enter the second operand: '))

    # get the third operand if they wanted three
    operand_3 = None
    if num_operands == 3:
        operand_3 = get_num(input('Please enter the third operand: '))

    # call the right function based on their operator and number of operands
    ...
    if operator == '5':
        if operand_3:
            print("Sorry, three operands and an exponent?")
        else:
